...
export function VoiceAgentProvider({ children }: VoiceAgentProviderProps) {
  ...
  const createSessionMutation = useMutation(
    api.lifeos.voiceagent.createSession,
  );
  ...
  // Only load once when disconnected to avoid excessive reactive queries
  const shouldLoadHistory =
    state.connectionState === "disconnected" && state.messages.length === 0;
  const recentMessages = useQuery(
    api.lifeos.voiceagent.getRecentMessages,
    shouldLoadHistory ? { limit: 50 } : "skip",
  );
  ...
  const connect = useCallback(
    async (roomName?: string) => {
      ...
      try {
        ...
        if (!userId) {
          console.warn(
            "[VoiceAgent] Warning: No userId available. Tools requiring user context will fail.",
          );
        }
        ...
        room.on(
          ...
          (track, publication, participant) => {
            if (
              ...
              console.log(
                "[VoiceAgent] Audio track subscribed, attaching for playback",
              );
        ...
        room.on(
          ...
          (segments, participant, publication) => {
            console.log(
              "[VoiceAgent] Transcription received:",
              segments,
              "from:",
              ...
              if (segment.final && segment.text.trim()) {
                ...
                if (sessionIdRef.current) {
                  addMessageMutation({
                    ...
                    console.error(
                      "[VoiceAgent] Failed to save message:",
                      error,
                    );
                  });
                }
