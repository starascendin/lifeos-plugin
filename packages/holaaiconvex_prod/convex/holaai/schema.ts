...
export const holaaiTables = {
  ...
  // User's personal vocabulary bank for saved translations
  hola_vocabBank: defineTable({
    userId: v.id("users"),
    sourceText: v.string(), // Original text (Spanish or English)
    translatedText: v.string(), // Translation
    sourceLanguage: v.string(), // "es" | "en"
    targetLanguage: v.string(), // "en" | "es"
    normalizedKey: v.string(), // Lowercase trimmed for deduplication
    context: v.optional(v.string()), // Source: "translate" | "chat_highlight"
    addedAt: v.number(), // Timestamp when added
  })
    .index("by_user", ["userId"])
    .index("by_user_normalized", ["userId", "normalizedKey"])
