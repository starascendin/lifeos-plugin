...
export const linkMeetingToCalendarEvent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Auto-match calendar attendees to contacts via unified contact system
    const attendees = (event.attendees || []).filter(
      (a) => !a.self && a.email
    );
    if (attendees.length > 0) {
      // Get user's email for self-filtering
      const userEmail = event.attendees?.find((a) => a.self)?.email;

      const meetingIdStr = args.meetingId as string;
      for (const attendee of attendees) {
        if (!attendee.email) continue;
        const normalizedEmail = attendee.email.toLowerCase().trim();

        // Skip self
        if (userEmail && normalizedEmail === userEmail.toLowerCase().trim()) {
          continue;
        }
        ...
        const emailRecord = await ctx.db
          ...
          .withIndex("by_user_email", (q) =>
            q.eq("userId", user._id).eq("email", normalizedEmail)
          )
          .first();
        ...
        // Link meeting to person in unified table (dedup)
        ...
        if (!existingLink) {
          await ctx.db.insert("lifeos_meetingPersonLinks", {
            ...
            confidence: 1.0,
            reason: `Matched by calendar email: ${normalizedEmail}`,
            createdAt: now,
          });
        }

        // Also dual-write to legacy table for backward compat
        const existingLegacyLink = await ctx.db
          .query("life_granolaMeetingPersonLinks")
          .withIndex("by_meeting", (q) =>
        ...
        if (!existingLegacyLink) {
          await ctx.db.insert("life_granolaMeetingPersonLinks", {
            ...
            aiConfidence: 1.0,
            aiReason: `Auto-matched by calendar email: ${normalizedEmail}`,
            createdAt: now,
          });
        }
      }
