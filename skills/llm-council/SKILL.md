...
node packages/lifeos-plugin/scripts/run-llm-council.mjs \
  ...
  --chairman "openai/gpt-5.4|GPT 5.4|openai"
```
...
provider as the third `|` field. Use `openai/gpt-5.4` with provider `openai`
for GPT 5.4 so it runs through the local OpenAI/Codex subscription, not Vercel.
...
- GPT 5.4 via OpenAI/Codex subscription: `openai/gpt-5.4`
