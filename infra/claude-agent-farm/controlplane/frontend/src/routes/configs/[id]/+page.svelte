<script lang="ts">
	...
	async function loadConfig() {
		...
		try {
			...
			formData = {
				...
				enabled_mcps: config.enabled_mcps || '',
	...
	async function handleSubmit() {
		...
		saving = true;
		try {
			await configs.edit(configId, formData);
			goto('/configs');
		} catch (err) {
	...
	async function handleDelete() {
		...
		if (confirm(`Delete config "${config.name}"?`)) {
			try {
				await configs.remove(configId);
				goto('/configs');
			} catch (err) {
				alert('Failed to delete config: ' + (err as Error).message);
			}
		}
	}
</script>
...
<div class="mx-auto max-w-4xl space-y-6 p-4 md:p-6">
	...
	<div class="flex items-center gap-3">
		...
		<div class="min-w-0 flex-1">
			<h1 class="text-xl font-bold sm:text-2xl">Edit Config</h1>
			<p class="text-sm text-muted-foreground">{loading ? 'Loading...' : config?.name || 'Unknown'}</p>
		</div>
	...
	{:else}
		<form on:submit|preventDefault={handleSubmit} class="space-y-4">
			...
			<FormSection title="Basic Info" description="Name and repository settings">
				<div class="space-y-4">
					<div>
						...
						<Input id="name" type="text" bind:value={formData.name} required placeholder="my-agent" />
					</div>
					...
					<div class="grid grid-cols-2 gap-4">
						...
						<div>
							...
							<Input
								...
								type="number"
								step="0.01"
								bind:value={formData.max_budget_usd}
							/>
			...
			<FormSection
				title="Advanced"
				description="Allowed tools and other settings"
				defaultOpen={false}
			>
				<div>
			...
			<div class="flex gap-3 pt-4 sm:hidden">
				...
				<Button class="flex-1" on:click={handleSubmit} loading={saving}>Save Changes</Button>
			</div>
		</form>
	{/if}
</div>
