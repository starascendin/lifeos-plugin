Track follow-ups needed with people and clients. Use lifeos-prod MCP tools.

1. Call `get_people` to get all contacts
2. Call `get_clients` to get all clients
3. Call `get_person_timeline` to see recent interactions
4. Call `get_beeper_threads` to see recent message activity
5. Call `get_granola_meetings` to see recent meetings
