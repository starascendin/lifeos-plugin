Check on neglected relationships. Use lifeos-prod MCP tools.

1. Call `get_people` to get all contacts
2. Call `get_person_timeline` to see interaction history
3. Call `get_beeper_threads` to check message activity
4. Call `get_granola_meetings` to see meeting history
